    ...
    <div className="min-h-screen bg-[#070A0F] text-slate-100 font-sans pt-24 pb-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          ...
          <span className="text-xs font-mono text-slate-400">Developer REST API Reference</span>
        </div>

        {/* Header */}
        <div className="bg-[#0B0F17] border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-900 border border-slate-800 text-xs font-mono font-bold text-amber-400">
            <Terminal className="w-3.5 h-3.5" />
            <span>KRYONHOST REST API V1</span>
          </div>
          <h1 className="text-3xl font-black text-white">API Reference & Endpoints</h1>
        ...
        <div className="bg-[#0B0F17] border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl font-mono text-xs">
          ...
          <p className="text-slate-400 font-sans text-xs">
            Authenticate all API requests by including your secret API token in the <code className="text-[#0096C7]">Authorization</code> header as a Bearer token.
          </p>
        ...
        {/* VPS Endpoints Card */}
        <div className="bg-[#0B0F17] border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl font-mono text-xs">
          <h2 className="text-sm font-bold uppercase tracking-wider text-slate-200 flex items-center gap-2">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={ep.path} className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex items-center gap-3">
                  <span className={`px-2 py-0.5 rounded font-black text-[10px] ${
                    ep.method === 'POST' ? 'bg-emerald-500/20 text-emerald-400' :
                    ep.method === 'GET' ? 'bg-blue-500/20 text-blue-400' : 'bg-rose-500/20 text-rose-400'
                  }`}>
                    {ep.method}
                  </span>
          ...
          <div className="space-y-2 pt-2">
            ...
            <pre className="p-4 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 overflow-x-auto text-[11px] leading-relaxed">
              {curlExample}
            </pre>
          </div>
        </div>
