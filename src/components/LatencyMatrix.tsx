  ...
  return (
    <section className="py-24 bg-slate-50 text-slate-900 font-sans border-b border-slate-200/90 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 relative z-10">
        ...
        {/* Test IP Box */}
        <div className="max-w-xl mx-auto bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between font-mono text-xs shadow-sm">
          <div className="flex items-center gap-2.5">
            <Activity className="w-4 h-4 text-emerald-600 animate-pulse" />
            <span className="text-slate-500">Test IP (Mumbai):</span>
            <span className="font-black text-slate-900 text-sm">{testIP}</span>
          </div>
          <button
            onClick={handleCopyIP}
            className="px-3.5 py-1.5 rounded-xl bg-slate-100 border border-slate-200 hover:bg-slate-200 font-bold text-slate-700 transition-colors flex items-center gap-1.5 cursor-pointer"
          >
            {copiedIP ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5 text-slate-500" />}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 font-mono text-xs">
          {regions.map((r) => (
            <div
              key={r.city}
              className="p-5 rounded-2xl bg-white border border-slate-200/90 space-y-3 hover:border-[#0096C7] hover:shadow-md transition-all shadow-2xs group"
            >
              <div className="flex justify-between items-center text-[10px]">
                <span className="text-slate-500">{r.status}</span>
                <span className="font-bold text-[#0096C7]">{r.isp}</span>
              </div>

              <div className="font-heading font-bold text-slate-900 text-sm group-hover:text-[#0096C7] transition-colors">{r.city}</div>

              <div className="flex items-baseline justify-between pt-2 border-t border-slate-100">
                <span className="text-slate-500 text-[10px]">Avg Latency:</span>
                <span className="font-black text-lg text-emerald-600">{r.ping}</span>
              </div>
            </div>
          ))}
        </div>
